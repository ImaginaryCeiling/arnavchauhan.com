...
tags: ['commentary']
draft: false
summary: 'Patience is a virtue. We are losing it.'
---
...
It felt annoying to actually write a long and detailed prompt. Following some introspection, I came to a pretty damning (yet obvious) conclusion about society: we are losing patience.

It shows in not just the way we write prompts for AI, but also in the way we interact with each other and complete tasks. In a world where it's so easy to have content made, we fail to put effort into creation. It begins with "Have _(AI tool of choice)_ make it", then becomes an automated email answerer, and will eventually start becoming the only medium for us to communicate in.
